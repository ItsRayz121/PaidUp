...
- **THE LIVE HOURGLASS FILL WAS BUILT, THEN REVERTED ON SIGHT (founder,
  2026-09-09).** The entry above says the fix for the retired coin split is "a
  render of the same scene with an EMPTY glass, which this component could then
  fill live". That was built — the pile was inpainted out of the render, the
  nine coins were cut from it as a 5KB alpha sprite, and `MiningHero` revealed
  one per ninth of the session — and the founder reverted it immediately on
  seeing it. **`/mine` is back to the fixed render, exactly as `168a9e4` left
  it**, and the empty/pile assets, the generated geometry and the build script
  are gone from the tree with it.
  - ⚠️ **DO NOT REBUILD IT FROM SCRATCH.** Everything is recoverable from
    commit `f419fc0` — `api/mine-hero-assets.cjs` (the inpaint + sprite cut,
    with its measured dilation of 14 and the reasons smaller values leave a
    pile-shaped ghost), both assets, and the component wiring. It was verified
    in real Chrome at every state and A/B'd against the render at full pile, so
    the work is sound; what it lacked was the founder wanting it.
  - **The standing rule is unchanged and now has evidence behind it**: the
