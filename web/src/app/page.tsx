    ...
    <div className="px-4 pt-5 pb-8 space-y-6">
        ...
        <Card className="overflow-hidden">
          <div className="bg-brand p-5 text-white">
            ...
            <p className="mt-1 text-white/80">
              That is about <span className="font-semibold text-white">{formatMoney(points)}</span>
            </p>
          </div>
          <div className="p-4">
            {canWithdraw ? (
              <Button href="/wallet/withdraw" variant="accent"><WalletIcon size={20} /> Get my money</Button>
            ) : (
              <>
                <div className="mb-2 flex items-center justify-between text-sm">
                  <span className="text-muted">{formatPoints(toGo)} points to your first payout</span>
      ...
      <Link href="/refer" className="block">
        <Card className="flex items-center gap-3 p-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-semibold text-brand-ink">{ref.data?.joined ?? 0} friends joined</p>
            <p className="text-sm text-muted">
              You earned <span className="font-semibold text-accent-ink">{formatPoints(ref.data?.earnedPoints ?? 0)} points</span> from them.
            </p>
          </div>
