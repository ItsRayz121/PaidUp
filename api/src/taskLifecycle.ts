export type StoredTaskStatus =
  | "draft" | "scheduled" | "active" | "paused" | "disabled" | "exhausted" | "ended" | "deleted";
...
export function campaignState(row: LifecycleRow, at = Date.now()): CampaignState {
  ...
  if (status === "draft" || status === "paused" || status === "exhausted"
...
export function unavailableMessage(state: CampaignState): string {
  ...
  if (state === "deleted") return "This task is no longer available.";
